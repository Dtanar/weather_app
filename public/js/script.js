...
const API_KEY = '7f542c1656bd4d77b64230642241105'; // Replace with your API key
...
function updateWeather(data) {
    ...
    currentWeatherContainer.innerHTML = `
        ...
        <div class="weather-icon">${getWeatherIcon(current.condition.text)}</div>
...
function cacheWeather(location, data) {
    localStorage.setItem(location, JSON.stringify({ data, timestamp: Date.now() }));
}
...
async function fetchWeather(location) {
    ...
    try {
        const response = await fetch(`${BASE_URL}/current.json?key=${API_KEY}&q=${location}=yes`);
        const data = await response.json();
        console.log(data)
